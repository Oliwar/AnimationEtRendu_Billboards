    ...
    QOpenGLWidget(parent)
{
    ...
    fumee = new Smoke(QVector3D(0, 0.5f, 0), 1.0f);
}
...
void GLArea::paintGL()
{
    ...
    fumee->animate(dt);
    fumee->display(program_particule, vbo_particule);
...
void GLArea::keyPressEvent(QKeyEvent *ev)
{
    ...
    switch(ev->key()) {
        ...
        case Qt::Key_Left :
            fumee->position.setX(fumee->position.x() - 0.5f);
            update();
            break;
        ...
        case Qt::Key_Up :
            fumee->position.setZ(fumee->position.z() - 0.5f);
            update();
            break;
